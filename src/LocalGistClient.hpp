...
public:
  ...
  LocalGistClient(const std::string& wavPath,
                  const std::string& outDeviceName = "Apple Inc.: MacBook Pro Speakers",
                  int _bufferSize = 2048,
                  int _nChannels = 1,
                  int _sampleRate = 48000); // defaults for saved Jamulus wav
  void stopRecording();
...
private:
  ...
  // NOTE: The WAV handle is touched from the audio callback and main thread (shutdown).
  // Use atomic exchange to guarantee it is closed exactly once.
  std::atomic<drwav*> segmentWavHandle { nullptr };
